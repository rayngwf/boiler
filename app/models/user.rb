...
class User < ActiveRecord::Base
  ...
  validates_presence_of :email_hash, :if => :not_using_openid?
  validates_uniqueness_of :email_hash, :if => :not_using_openid?
  ...
  def self.authenticate(email, password)
    u = find_in_state :first, :active, :conditions => { :email_hash => self.hash_email(email) } # need to get the salt
    u && u.authenticated?(password) ? u : nil
  end

  def email=(value)
    value = value ? value.downcase : nil
    write_attribute :email, value
    write_attribute :email_hash, value ? Facebooker::User.hash_email(value) : nil
  end

  def name
    (value = read_attribute(:name)).blank? ? read_attribute(:email) : value
  end
  ...
  def self.hash_email(email)
    Facebooker::User.hash_email(email)
  end
